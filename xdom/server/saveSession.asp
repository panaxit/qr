...
file_location=parent_folder&"user_"&session("user_id")&".xml"
...
<x:message xmlns:x="http://panax.io/xdom" type="exception"><%= message %></x:message>
...
ELSE
    IF INSTR(content_type,"xml")>0 THEN %>
<x:message xmlns:x="http://panax.io/xdom" type="success"><%= message %></x:message>
<%  ELSEIF INSTR(content_type,"json")>0 THEN
    Response.ContentType = "application/json" %>
{"status":"success"}
<%  ELSE 
