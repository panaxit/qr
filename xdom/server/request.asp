...
<x:message xmlns:x="http://panax.io/xdom" x:id="message_<%= REPLACE(REPLACE(REPLACE(NOW(),":",""),"/","")," ","_") %>" type="exception"><%= REPLACE(REPLACE(message,">","&gt;"),"<","&lt;") %></x:message>
...
DIM data_filters: data_filters = Request.ServerVariables("HTTP_X_DATA_FILTERS")
...
IF data_predicate="" AND request.querystring("predicate")<>"" THEN
    data_predicate = request.querystring("predicate")
END IF
IF data_predicate="" AND data_filters<>"" THEN
    data_predicate=data_filters
END IF
IF data_predicate="" AND request.querystring("filters")<>"" THEN
    data_predicate = REPLACE(request.querystring("filters"),"'","''")
END IF
strSQL = command
IF data_fields="" THEN
    data_fields = request.querystring("fields")
END IF
...
IF 1=0 and fso.FileExists(file_location) THEN
    ...
    Response.end
END IF
DIM sRequestType: sRequestType="SET NOCOUNT ON; IF OBJECT_ID('#Object.FindObjectsInQuery') IS NOT NULL BEGIN SELECT TOP 1 [Type], [Object_Name] FROM #Object.FindObjectsInQuery('"&REPLACE(command,"'","''")&"') ORDER by Position END ELSE BEGIN SELECT [Type]=NULL, [Object_Name]=NULL WHERE 1=0 END"
'response.write "<!-- "&sRequestType&" -->"
...
IF INSTR(sType,"T")<>0 THEN
    ...
    IF data_value<>"" THEN
        IF fields<>"" THEN
            fields=fields&","
        END IF
        fields=fields&"[x:value]="&data_value
    END IF
    IF data_text<>"" THEN
        IF fields<>"" THEN
            fields=fields&","
        END IF
        fields=fields&"[x:text]="&data_text
    END IF
    IF data_fields<>"" THEN
        IF fields<>"" THEN
            fields=fields&","
        END IF
        fields=fields&data_fields
    END IF
    IF fields="" THEN
        fields="*"
    END IF
    fields="TOP 1000 "&fields&" "
...
If Request.TotalBytes > 0 Then
    ...
    ELSE
        ...
        sParameters=payload_parameter_name&"='"&REPLACE(payload,"'","''")&"'"
    END IF
End If
IF INSTR(sType,"P")<>0 OR INSTR(sType,"F") THEN
    ...
    IF NOT(detect_input_variables OR detect_output_variables) THEN
        data_predicate=sParameters
    ELSE
        IF sParameters<>"" AND request.querystring("Parameters")<>"" THEN
            sParameters=sParameters&", "
        END IF
        sParameters = sParameters&URLDecode(request.querystring("Parameters"))
        DIM sParameter, ns
        ...
        IF detect_input_variables THEN
            DIM sSQLXMLParams: sSQLXMLParams="EXEC #panax.parameterStringToXML '"&REPLACE(sParameters,"'","''")&"'"  
            IF debug THEN
        ...
        END IF
        DIM sSQLParams: sSQLParams="DECLARE @parameters XML; EXEC [#panax].[getParameters] '"&REPLACE(sRoutineName,"'","''")&"', @parameters=@parameters OUT"&rebuild_parameters_snippet&"; SELECT @parameters"  
        IF debug THEN
            response.ContentType = "text/xml" 
        ...
        IF NOT(rsParameters.BOF AND rsParameters.EOF) AND rsParameters.fields.Count>0 THEN
	        ...
	        IF NOT(xmlOutputParameters.documentElement IS NOTHING) THEN
		        ...
		        FOR EACH oNode IN xmlOutputParameters.documentElement.selectNodes("/*/*")
                    ...
                    IF NOT(xParameter IS NOTHING) THEN
                        sParameterValue = xParameter.Text
                        sParameterType = xParameter.getAttribute("xsi:type")
                    ELSEIF request.querystring(sParameterName)<>"" THEN
                        sParameterValue = request.querystring(sParameterName)
                    ELSE
                        sParameterValue = "DEFAULT"
                    END IF                
                    
                    IF oNode.getAttribute("isRequired")=1 AND oNode.getAttribute("isOutput")=0 AND (xParameter is nothing OR sParameterType<>"string" AND sParameterValue="" OR sParameterValue="DEFAULT") THEN
                        missingParameters = TRUE
                        sParameterValue=""
                        oNode.setAttribute "missing", "true"
                    ELSEIF (xParameter IS NOTHING) AND oOtherNodes.length>0 THEN
        ...
        IF INSTR(sType,"F")<>0 THEN
            data_predicate = "(" & TRIM(sParameters) &")"
        ELSE
            data_predicate = TRIM(sParameters)
        END IF
...
<x:prompt xmlns:x="http://panax.io/xdom"><%= xmlOutputParameters.xml %></x:prompt>
<%
            response.end
        END IF
    END IF
    ...
    IF INSTR(sType,"P")<>0 THEN
        strSQL="EXEC "&sRoutineName& " " & data_predicate &"; "
        IF sOutputParams<>"" THEN 
            strSQL=strSQL&"WITH XMLNAMESPACES('http://panax.io/xdom' as x, 'http://panax.io/xdom/binding/source' as source, 'http://www.mozilla.org/TransforMiix' as transformiix) SELECT (SELECT "&sOutputParams&" FOR XML PATH(''), TYPE) FOR XML PATH(''), ROOT('x:response'), TYPE"
        END IF
    ELSEIF INSTR(sType,"T")<>0 THEN 'Table  y Table Function
        strSQL="(SELECT "&fields&" FROM "&sRoutineName&" "&data_predicate&" ORDER BY 1 FOR XML PATH('x:r'), TYPE)"
        strSQL="SET NOCOUNT ON; WITH XMLNAMESPACES('http://panax.io/xdom' as x, 'http://panax.io/xdom/binding/source' as source, 'http://www.mozilla.org/TransforMiix' as transformiix) SELECT "&strSQL&" FOR XML PATH('')"& root_node &", TYPE"
    ELSEIF INSTR(sType,"F")<>0 THEN
        strSQL="SELECT "&sRoutineName & data_predicate
    ELSE
        strSQL="SELECT "&fields&" FROM "&sRoutineName & " AS Result "&data_predicate
    END IF
...
DO
    ...
    <<%= Request.ServerVariables("HTTP_ROOT_NODE") %> xmlns:x="http://panax.io/xdom" xmlns:source="http://panax.io/xdom/binding/source" />
    <%                  ELSE
    ...
    ELSE 
        IF INSTR(content_type,"xml")>0 THEN
            ...
            response.ContentType = "text/xml" 
        %><?xml-stylesheet type="text/xsl" href="message.xslt" role="message" target="body" action="append"?>
<x:message xmlns:x="http://panax.io/xdom" x:id="message_<%= REPLACE(REPLACE(REPLACE(NOW(),":",""),"/","")," ","_") %>" type="success">El proceso ha terminado</x:message>
        <% ELSE %>
	        this.status='success'
	        this.recordSet=new Array()<% 
        END IF
